      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-3 px-4 py-3 sm:px-6 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex items-center justify-between gap-5">
          <Link className="flex items-center gap-2 text-sm font-semibold tracking-tight" href="/">
            <span className="grid size-8 place-items-center  border border-line bg-panel text-accent">
          ...
          <nav className="hidden items-center gap-1 sm:flex">
            {navItems.map((item) => (
              <Link
                className={` px-3 py-2 text-sm transition ${pathname === item.href ? "bg-panel-lift text-text" : "text-muted hover:bg-panel hover:text-text"}`}
                href={item.href}
                key={item.href}
              >
                {item.label}
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
          <div className="relative sm:w-80">
            ...
            <input
              className="w-full  border border-line bg-panel px-9 py-2.5 text-sm text-text outline-none transition placeholder:text-faint focus:border-line-strong"
          ...
          <div className="relative">
            <button
              className="inline-flex w-full items-center justify-center gap-2  border border-line bg-panel px-3 py-2.5 text-sm font-medium text-text transition hover:border-line-strong sm:w-auto"
          ...
          <button
            className="inline-flex items-center justify-center  border border-line bg-transparent px-3 py-2.5 text-sm text-muted transition hover:text-text"
            onClick={logout}
            type="button"
          >
